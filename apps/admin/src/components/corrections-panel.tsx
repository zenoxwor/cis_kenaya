...
interface Correction {
  ...
  record: {
    date: string;
    student: { studentNo: string; firstName: string; lastName: string };
    class: { name: string };
  };
  requestedBy: { displayName: string; username: string };
  approvedBy: { displayName: string; username: string } | null;
}
...
export default function CorrectionsPanel({ canApprove }: Props) {
  ...
  return (
    <div>
      ...
      {loading ? (
        <p style={{ color: BRAND.textMuted }}>Loading corrections…</p>
            ...
            <tbody>
              {corrections.map((c, idx) => (
                ...
                >
                  ...
                  <td style={tdStyle}>{c.requestedBy.displayName}</td>
                    ...
                    <td style={tdStyle}>
                      {c.status === "PENDING" ? (
                        ...
                        <span style={{ fontSize: "0.75rem", color: BRAND.textMuted }}>
                          {c.approvedBy ? `By ${c.approvedBy.displayName}` : "—"}
                        </span>
                      )}
                    </td>
                  )}
